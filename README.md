...
Topological Boundary Line Estimation using Recurrence of Neighbouring Emissions, or TOBLERONE, is a topological data analysis algorithm which uses persistent homology to identify objects in monochrome images. The programme is run entirely through RStudio and requires only the **tiff** and **grid** packages to function.
## Installation
Full instructions for installing and using TOBLERONE are available from the video above. Alternatively, written instructions are given below. The package can be installed using pre-existing functions in the **devtools** (add link here) package. It is highly recommended that you use **RStudio** (add link here). Simply enter the line below into the command prompt.
```{r}
...
```
From here, follow the instructions given in the command prompt or read the additional information below.
...
* **base_window_size** – An integer value which describes the number of pixels that will be includes in both the i and j directions on either side of the image. The smaller this is, the more compact the window will be around each segmented image and the faster the computation time.
* **identify_boundary** – Boolean value which dictates whether legacy code will be performed to identify object boundaries and the layers beneath.
* **smooth_image** – Boolean value which dictates whether identified objects will be smoothes by removing rogue pixel-wide protrusions during boundary identification. Recommended to be kept true.
